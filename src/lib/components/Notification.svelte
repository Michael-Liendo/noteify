<script lang="ts">
	...
	$: {
		...
		figureClass = classNames(
			...
			notification.kind === NotificationKind.Success && 'text-white bg-green-400',
			notification.kind === NotificationKind.Warning && 'text-white bg-amber-400 ',
...
<li class={notificationClass}>
	...
	<button
		on:click={() => notifications.close(notification.id)}
